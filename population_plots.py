...
def plot_basic(d,frequency='H'):
    ...
    for idx in d.keys():
        ...
        ax1[idx].plot(df2_h.index,df2_h['MET_MVPA'], label='MET_MVPA',linewidth=3, color ='lightgreen',alpha=1)
        ax1[idx].plot(df2_h.index,df2_h['MET_VigPA'], label='MET_VigPA',linewidth=3, color ='forestgreen',alpha=1)
        ax1[idx].set_ylabel('MET_MVPA')
        ax1[idx].legend()
        
        #Add grey windows for sleep
        for i in range(len(d[idx].sleep_rec)):
            ax1[idx].axvspan(d[idx].sleep_rec['sleep_onset'][i],d[idx].sleep_rec['sleep_offset'][i],facecolor='cornflowerblue',alpha=0.4)
        for j in range(len(d[idx].crespo_on)):
            ax1[idx].axvspan(d[idx].crespo_on[j],d[idx].crespo_off[j],facecolor='royalblue', alpha=0.3)
...
def plot_daily(d,i):
    ...
    for idx in range(len(DFList)):
        ...
        #ax1[idx].grid(color='#b2b2b7', linestyle='--',linewidth=1, alpha=0.5)
        ax1[idx].plot(df2_h.index, df2_h['ACC'],label='ACC', linewidth=3, color ='black',alpha=1)
        ax1[idx].fill_between(df2_h.index, 0, df2_h['min_MVPA']*10,facecolor ='lightgreen',alpha=0.7, label='MVPA',edgecolor='lightgreen')
        #ax1[idx].plot(df2_h.index,df2_h['MET_VigPA'], label='MET_VigPA',linewidth=1, color ='black',alpha=1)
        ax1[idx].fill_between(df2_h.index, 0, df2_h['min_VigPA']*10,facecolor ='forestgreen',alpha=0.7, label='VPA',edgecolor='forestgreen')
        
        ax1[idx].fill_between(df2_h.index, 0, (df2_h['MET_Sed'])*10,facecolor ='lemonchiffon',alpha=0.6,label='sedentary',edgecolor='lemonchiffon')
        ax1[idx].fill_between(df2_h.index, 0, -(df2_h['sleep_window_0.4']-2)*10,facecolor ='cornflowerblue',alpha=1,label='sleep')
